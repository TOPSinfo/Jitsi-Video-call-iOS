...
class OTPViewController: UIViewController, UITextViewDelegate {
    ...
    func pinEntered() {
        ...
        otpViewModel.OTPViewControllerDelegate?.buttonClicked(verificationId: self.verificationId,
                                                              code: pinView.getPin())
...
extension OTPViewController: FirebaseAuthViewModelDelegate {
    ...
    func login(_ authResult: AuthDataResult?) {
        if let userDetail = authResult {
            ...
            otpViewModel.OTPViewControllerDelegate?.checkUserAvailability(userID: uid)
        }
    }
    ...
    func checkAvalability(_ isRegisterd: Bool) {
        ...
        if isRegisterd {
            ...
            let fbcfm = otpViewModel.firebaseViewModel.firebaseCloudFirestoreManager
            
            fbcfm.getUserDetail(userID: Auth.auth().currentUser!.uid) { (_) in
                ...
                Singleton.sharedSingleton.navigate(from: self, toWhere: vcUL,
                                                   navigationController: self.navigationController)
            } failure: { (error) in
                print(error)
            }
        } else {
            guard let vcRU = Singleton.sharedSingleton.getController(storyName: Singleton.StoryboardName.Main,
            controllerName: Singleton.ControllerName.RegisterUserVC) as? RegisterUserVC else { return }
            Singleton.sharedSingleton.navigate(from: self, toWhere: vcRU,
                                               navigationController: self.navigationController)
        }
    }
}
