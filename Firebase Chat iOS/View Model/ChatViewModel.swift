...
extension ChatViewModel : ChatVCDelegate {
    ...
    func uploadVideo(_ files: [(filename: String, file: Data, type: MediaType)]) {
        ...
        group.notify(queue: .main) {
            ...
            if videoTask == nil {
                ...
                self.firebasechatViewModelDelegate?.didUploadVideo?(imageRef, imageTask: imageTask, videoRef: videoRef, videoTask: videoTask)
            }
        }
    ...
    func uploadMediea(_ filename: String, file: Data, type: MediaType) {
        ...
        Singleton.sharedSingleton.showLoder()
        firebaseViewModel.firebaseStorageManager.uploadImage(childPath: filename, imageNeedstoUpload: file, type: type) { task,url  in
            Singleton.sharedSingleton.hideLoader()
    ...
    func sendMessage(conversationID:String, messageID: String, dic: [String : Any]) {
        ...
        firebaseViewModel.firebaseCloudFirestoreManager.sendMessage(conversationID: conversationID, messageID: messageID, dic: dic) { message in
    ...
    func getMessageList(_ touserID: String, isForGroup: Bool) {
        ...
        Singleton.sharedSingleton.showLoder()
        firebaseViewModel.firebaseCloudFirestoreManager.getConversation(toUserid: touserID, isForGroup: isForGroup) { (messases) in
